...
>
> **关键定位**:交付物是 `/deliver` 的**版本冻结产物**,不手动编辑。本规范是交付物的「模板」,不是又一份要手维护的源文件。
> 领域 `design.md` 是当前设计事实源；领域 `delivery/prd.md` 是从 Design 生成的长期业务化投影；`/deliver` 再从长期 PRD、Design、调研与版本信息组装本规范的各章节。
> 业务变化先更新对应领域 `design.md`，重新生成长期 PRD 后再触发 `/deliver`。
...
`/deliver` 生成本文时,各章节从下列来源拉取或计算。长期 PRD 已完成去工程化转译，版本快照不得重新引入 Design 中的工程细节:

| 本规范章节 | 组装来源 |
| --- | --- |
| 本版变更摘要 / 变更日志(§1) | 生成:`git tag release/vX` + 领域长期 PRD 按 `## P-XXX` diff |
| 名词解释(§2) | 领域长期 PRD |
| 业务现状与痛点(§3.1)、核心目标(§4) | 领域长期 PRD |
| 前期调研(§3.2) | `project/research/` 摘要 + 链接 |
| 需求优先级(§3.3)、功能清单(§5.1) | 生成:由长期 PRD 条目和需求总览汇总 |
| 不在本期范围(§5.2) | 长期 PRD 的领域边界 + 版本级补充 |
| 设计概述(§6,原则 / 整体流程) | 长期 PRD 的业务流程；按需链接 `design.md` |
| 详细需求(§7) | **领域长期 PRD 的 `P-XXX` 条目**(完整摘录) |
| 非功能性需求(§8) | 领域长期 PRD |
...
按功能模块分节,每个模块包含以下内容(按需裁剪)。本章由 `/deliver` **完整摘录领域长期 PRD 的 `P-XXX` 条目**，条目结构见 `external-prd.md`「详细需求条目模板」。
