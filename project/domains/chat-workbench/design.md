...
1. **左列 — 会话分组与搜索**
   - 顶部:筛选条 — `全部号` / `按号筛选`(下拉,从有权限的号里选)
   - 中部:**会话分组列表(默认聚合所有有权限的号)**;每条会话卡片显示玩家昵称 + 所属企微号小徽标,按 `排队中 / 会话中 / 已结束` 折叠
   - 底部:综合搜索入口
2. **中列 — 会话主视图**
...
| 企微号控制台页 | `/control` | 客服 | 管理企微号:扫码登录 / 状态监控 / 云桌面人工介入 | 顶部条 / 左列(企微号选择器)/ 主体(云桌面投屏 + 控制条) | 顶栏一级 tab,与 `/workbench` 并列;支持 `?focus=<企微号 id>` |
...
│           │ ┌────────────────────────────┐ │                     │
│ 筛选条   │ │ 会话标题栏 + 操作条 (52px)  │ │ 玩家档案挂载位       │
│ 全部号▼  │ ├────────────────────────────┤ │ (player-center 领域) │
│           │ │ 消息流                      │ │                     │
│ 排队中(3) │ │ ↑ 历史                      │ │  - 玩家头像/昵称     │
│ ┌───────┐│ │ ↓ 最新                      │ │  - 备注/标签         │
│ │卡片   ││ │                              │ │  - 自定义字段       │
...
| 字段 | prop | 控件 | 行为 |
| --- | --- | --- | --- |
| 号筛选 | `accountId` | 单选下拉 | 默认"全部号"(聚合视图);选具体号后会话卡片只显示该号下的;切换不保留滚动位 |
...
| 区域 | 内容 |
| --- | --- |
| 左侧 | 玩家头像 + 昵称 + 企微号徽标 + 当前指派人(无 → "未指派"灰色) |
| 右侧操作条 | 指派 / 转接 / 标记 / 置顶 / 结束会话 / 更多 ⋯(更多里放复制 ID) |

按钮启用规则:

- 当前会话指派人 ≠ 当前客服 → 转接/标记/置顶/结束 全部禁用(只读)
- 指派人为空 → 显示"指派"按钮;否则显示"转接"
- 已结束分组的会话:全部按钮禁用,仅可查看历史
...
| 切号筛选 | 左列下拉 | 重渲染会话列表 | 当前打开的会话若属其他号 → 二次确认是否关闭 | — |
...
| 切换号导致会话被关 | 二次确认弹窗"切换后将关闭当前会话",可取消 |
...
- **页面出口**:顶栏切回"工作台";顶部条"返回工作台"按钮(按上下文带回原会话)。
- **本页负责**:扫码登录企微号、号状态详细监控、云桌面视频流投屏与操作
...
| 控制台顶部条 | 100% × 40px | 添加号 + 当前号信息 + 返回工作台 |
| 左列 | 240px | 号卡片列表 + 底部 + 添加号 入口 |
| 主体 | flex | 视频流投屏 + 底部控制条 |
| 控制条 | 100% × 40px | 视频操作按钮组 |
...
| 元素 | 行为 |
| --- | --- |
| `+ 添加号` | 启动新企微号扫码登录流程(主体投屏切换为新桌面 + 二维码) |
| 当前选中号信息 | 显示"号简称 + 状态点 + 在线时长";状态点颜色随实时变化 |
| 返回工作台 | 路由跳 `/workbench`;若是从工作台跳来的(referer 或上下文)则回到原会话 |
...
| --- | --- | --- |
| 🔄 刷新 | 重连视频流 | 任何状态 |
| ⛶ 全屏 | 视频区铺满浏览器窗口(Esc 退出) | 视频流已建立 |
| ⟲ 重新登录 | 等同号卡片右键的"重新登录" | 当前号在线/离线均可 |
| ➜ 跳工作台对应会话 | 携带 `?fromControl=<号>` 跳 `/workbench`,自动选中该号最近一条会话 | 任何状态 |
...
| 添加号 | 顶部条 / 列表底"+" | 启新桌面 + 打开企微 + 扫码遮罩 | 扫码成功 → 号变在线 + 加入左列 | 超时 → 遮罩内提示重试 |
| 全屏 | 控制条 ⛶ / F | document.fullscreenElement 切换 | 视频区填满窗口 | — |
| 返回工作台 | 顶部条按钮 | 路由跳转 | 回到原会话 | 原会话已结束 → 落到默认欢迎页 |
...
- 工作台筛选条切单号视图 → 中列若正在打开某个会话且该会话属于其他号,需提示"切换后将关闭当前会话"并二次确认。
