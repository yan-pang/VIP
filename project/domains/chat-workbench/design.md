...
>
> - 需求业务背景 / 业务诉求 / 概念说明
> - 功能现状:功能当前状态、数据当前状态(数据源 / V1 策略 / 缺口)
...
2. **中列 — 会话主视图**
   ...
   - 主体:消息流(按时间倒序,带发送状态标识:已送达 / 失败;展示口径见「消息记录展示口径」)
...
- **发送消息**:客服在**指派给自己**的会话(或已结束会话的主动发起态)输入区输入 → 违禁词客户端校验 → 发送按钮点击 → 后端调用 RPA SDK → RPA 在云桌面执行 → 拿到企微返回内容后,将消息标为"已送达"或"发送失败"。发送前不再改动指派人(排队中会话已在接入时完成指派)。
...
- **消息撤回**:客服在自己已送达的消息上**悬停浮出「撤回」图标**(位于气泡侧的悬停操作条,后续可扩展更多图标)→ 点击 → V1 弹文字提示"将跳转控制台手动撤回" → 跳 `/control?focus=<企微号>` 在云桌面手动操作。
...
| 会话分组 | R028 | 排队中(指派人为空)/ 会话中(**仅指派给自己**)/ 他人接待中(进行中但指派人是他人,只读)/ 已结束;分组跃迁由指派动作和结束动作驱动,不由点击会话驱动 | 依赖会话指派事项 |
...
| 消息撤回入口 | R045 | 消息悬停浮出「撤回」图标 → 弹文字提示 → 跳 `/control?focus=<企微号>` 在云桌面手动操作 | 无 |
...
stateDiagram-v2
    [*] --> 排队中: 玩家首条消息到达
    [*] --> 会话中: 玩家中心对无会话关系"主动发起"<br/>直接创建 active 占位并指派当前客服
...
- `[*] → 会话中`:玩家中心对**无任何会话**的关系点"主动发起"时,工作台直接创建一条 `active` 占位并指派当前客服(详见共享规则 §1);这是除 `[*] → 排队中` 之外的另一条会话创建入口。
- 玩家长时间沉默不自动变更状态,会话仍留在"会话中";是否要沉睡 / 清理逻辑留给 v1.1+ 讨论。

### 图 2:消息发送完整流程

> 前提:能进入输入区发消息的会话必然是**已指派给自己的会话中**或**已结束会话的主动发起态**(排队中会话输入区不渲染)。因此发送前不再有"是否已指派"的分支,也不再有隐式指派首条失败回滚。

```mermaid
graph TD
    A[客服在可输入会话输入消息] --> B{违禁词校验}
    B -- 命中 --> B1[红字提示命中词<br/>发送按钮禁用]
    B -- 通过 --> D[调用阿里云 RPA SDK]
    D --> E[RPA 在云桌面执行]
    E --> F{企微返回内容匹配}
    F -- 30s 内匹配成功 --> G[消息标记已送达]
    F -- 超时或失败 --> H[消息标记发送失败<br/>会话状态/指派人不变更]
    H --> I{失败类型}
...
> 主动发起态例外:已结束会话主动发起时,首条**成功**才跃迁"会话中"并指派当前客服,失败保持已结束(见共享规则 §1),这条成功跃迁不体现在上图的普通发送分支里。
...
- **会话 ID 跨轮次保持不变**:同一个企微号 × 同一个玩家的会话,无论"已结束 → 排队中"或"已结束 → 会话中(主动发起)"重开多少次,都复用同一个 `conversationId`,**不**生成新会话;消息流也累积在同一个会话里、跨轮次按时间顺序展示。
...
- **消息发送失败不改状态机**:进入"会话中"(或已结束主动发起态)后,任何消息发送失败都不改变会话状态和指派人 —— 客服自己负责重发或介入。
- **客服主动发起(已结束)→ 发送成功才跃迁**:已结束会话点击"主动发起会话",输入区解锁但状态机和指派人**先不变**。仅当首条消息发送**成功**时,才把状态置为"会话中"并把指派人写为当前客服(等价于一次显式指派,在 `assigneeHistory` 里以 `reason='explicit'` + `note='客服主动发起'` 留痕)。发送失败 → 状态保持"已结束",指派人不变,输入区维持解锁可继续重试(成功之前根本没指派过,故也不存在任何回滚)。
...
- **从玩家中心"主动发起"跳入(`?playerId=&accountId=`)**(2026-05-30 补,占位语义 2026-05-30 二次调整):玩家中心(player-center)在某条(玩家×企微号)关系**无任何会话**时提供"主动发起"入口,跳 `/workbench?playerId=<pid>&accountId=<aid>`。工作台收到后:
  - 若该 `(playerId, accountId)` **已存在会话** → 定位该会话;若其状态为"已结束"则自动进入主动发起态(等价于点了"主动发起会话")。
  - 若**完全无会话** → 创建一个**「会话中」会话占位**(`status='active'` + 指派当前客服,`assigneeHistory` 留 `reason='explicit'` + `note='客服主动发起'`),立即落在左列"会话中"分组、有选中态、按 `lastMessageAt` 排在非置顶项最前(即发起时间)。客服随后在该会话内直接发首条消息:
    ...
    - 落库判定 = 该主动发起会话至少有一条发送成功(`sent`)的客服消息。真实链路由会话存档 API 承担,此处 mock 用 localStorage 模拟"落库后跨刷新可见"。
  - 被玩家删除(`removed_by_player`)的关系在玩家中心侧即禁用"主动发起"入口,不会跳入工作台。
  - 注:工作台内对**已结束会话**点"主动发起"仍走原"保持 `ended` 显示 → 首条发送成功才跃迁 active"路径,与此处玩家中心入口(乐观 `active`)UI 表现不同,但都不为失败的首条消息额外回滚已展示的会话。
...
- **左列分组按"是否指派给我"拆分**:进行中会话里,只有**指派给当前客服**的落入"会话中"(能回复);指派给其他客服的落入独立的"**他人接待中**"分组(默认折叠、只读查看)。目的:避免自己无法处理的会话堆在"会话中"里干扰,导致真正该我回的会话被漏掉。
...
- **处理**:
  ...
  - 输入区渲染守卫:排队中(未指派)/ 非自己指派 / 已结束非主动发起态 → 输入区不渲染
  - 违禁词出站拦截:本地校验 + 后端兜底
...
- 指派人为空(排队中)→ 显示"指派"按钮,且**输入区不渲染**(必须先指派接入才能回复,详见 1.5.6);否则显示"转接"
...
- 消息气泡:
  ...
  - **悬停操作条**:鼠标悬停在**自己已送达**的消息气泡上时,气泡侧边浮出一条**悬停操作条**(默认隐藏),当前只含「撤回」图标(`RollbackOutlined`);收到 / 系统 / 发送中 / 已失败的消息不显示。点「撤回」走撤回引导(R045)。**该操作条按可扩展设计**——后续可在此追加更多消息级悬停交互(如复制、引用、转发等),不必再引入右键菜单。
- 媒体消息:图片缩略,点击放大;视频带播放按钮缩略图;文件显示文件名 + 大小 + 下载图标
- **图文合并气泡**:一条消息可同时带文字 + 多个附件(图片 / 视频 / 文件),在同一气泡内竖排展示——图片走网格(多图换行、点击灯箱预览),视频 / 文件卡片限宽竖排,文字在最下方;附件区整体限宽,不横向撑破气泡。旧的单媒体消息(seed / 存档回拉)仍按单类型渲染,兼容不变。
...
- **跨轮次会话分割条**:同一会话多轮重开时,在消息流中按时间顺序插入系统消息作分割,跨轮次的历史消息**仍然保留**(同一 conversationId,见状态机规则)。具体三类分割条:
  ...
  - 客服在已结束会话点主动发起且首条消息发送成功(`已结束 → 会话中`)→ `客服于 YYYY-MM-DD HH:mm 主动发起会话`
  - **相邻边界合并展示(展示层口径,不改数据模型)**:当「本次会话已结束」与紧随的「重新发起 / 主动发起」两条系统条相邻出现(展开历史跨越轮次边界时),合并为**一条**分割线:`会话已于MM-DD HH:mm结束 · 玩家/客服MM-DD HH:mm重新发起/主动发起`。**结束与重开两端都带日期**(避免"结束于几点、哪天"读不出)。合并条已内联新一轮日期,其后同自然日的消息**不再重复插日期分割线**;未展开历史、最新一轮仅剩单独一条重开条时,该重开条同样内联完整日期并抑制紧随的冗余日期线。目的是避免"日期线 + 结束条 + 重开条 + 日期线"多枚分隔条在边界处堆叠。
- **历史轮次:默认只渲染最新一轮,上滚自动展开 + sticky 收起按钮**:
  ...
  - **轮次切分规则**:以 `本次会话已结束` 系统分割条作为轮次边界,该分割条本身归属上一轮的尾;`玩家于...重新发起` / `客服于...主动发起` 分割条归属下一轮的开头(因为它们描述的是新一轮的来源)。
...
**图文合并 + 草稿附件**:图片 / 视频 / 文件不再"选完即发",而是先进入文本框上方的**草稿附件预览条**;客服可攒多个附件、配文字,点一次发送作为**一条图文消息**发出(见共享规则 §5 与 P-114)。发送成功后草稿区清空;切换会话时草稿(文字 + 未发送附件)一并清空。
...
| 发送按钮 | 主色;**文字非空或有草稿附件时可点**;违禁词命中时禁用并红字提示 |
...
| 转接 | 点转接 → 选客服 | 修改指派人 | 玩家无感,该会话从我的"会话中"移入对方视角;在我这边落入"他人接待中" | 目标客服未登录 → 提示但允许 |
...
| 撤回 | 消息悬停点侧边「撤回」图标 | 弹文字提示 | 跳 `/control?focus=<号>` | — |
...
- 点击切换主体投屏号
- 当前选中卡片左边主色实线条 + 浅背景
- `重新登录` / `删除号`(后者带二次确认)走主体底部控制条,号卡片不提供右键菜单
- 列表底部 `+ 添加号` 主按钮入口
- 当前客服无权限的号不在列表中
...
| ⟲ 重新登录 | 进入扫码态(重新登录当前选中号) | 当前号在线/离线均可 |
...
| 删除号 | 控制条"删除号" | 二次确认 → 解绑云桌面 + 移除授权 | 左列移除该号 + toast | 后端拒绝 → toast |
...
- **处理**:列表过滤(**仅保留在线客服**——离线客服不支持指派 / 转接;指派模式可含自己,转接模式排除自己)
...
列表**仅含在线客服**(离线客服已在数据流过滤阶段剔除)。排序规则:**指派模式** = 自己最前 + 会话量少在前;**转接模式** = 会话量少在前(排除自己)。
