...
3. **消息管理**:`/messages` 列表里检索"会话",**以会话轮次(roundId = conversationId#N)为维度**(同一会话多轮拆多行),按对话管家(企微号)、玩家、最后消息内容 / 时间筛选;点某轮打开右侧 Drawer **只读浏览该轮消息**;"去工作台接待"仍按整会话跳转。
4. **跨页一致**:玩家管理列表行 / 玩家详情页 / 工作台 slot 三处共享同一份玩家与关系数据,备注 / 描述 / 标签 / 关系状态在任一处修改都跨页生效;企微原生备注 / 描述 / 标签由企微 API 双向同步,**ChatFlow 端为权威源**。
5. **接待与检索分离**:玩家管理 / 消息管理 / 玩家详情页 / Drawer **均不渲染输入区**,需要发消息或继续接待 → 跳工作台对应会话;若工作台不存在该会话(全新 / 已结束),进入工作台"主动发起会话"态,首条消息发送成功后会话才落库,与 chat-workbench 的"主动发起"语义对齐。
6. **工作台 slot ≠ 玩家详情页**:工作台右列 slot 是**接待场景专用的精简档案**(只装与本次接待最相关的字段:当前管家备注 / 描述 / 标签 / 关系状态 / 跨号 chip / 关键自定义字段),内容比 `/players/:id` 少且聚焦,不复用同一组件;有"在玩家管理打开"链接(新标签页)跳详情页看完整档案。
...
| 概念                   | 定义                                                                                                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 玩家                   | 一个真实自然人;在 ChatFlow 内由唯一 `playerId` 标识(底层来自企微 `external_userid`)。一个玩家可被多个企微号添加为好友。                                                                                             |
| 企微号(管家)              | 接待玩家的企业微信账号,本领域两个列表都以"企微号"为关键维度;一个企微号可由多名客服(坐席)轮班共用接待。`/players` 的"管家"列、`/messages` 的"对话管家"展示的都是企微号名(如"无尽冬日-玲奈");业务部门也常称之为"管家"。                                               |
| 客服(坐席)               | 真正登录 ChatFlow 操作消息的人(由 [permission](../permission/design.md) 维护);客服 ↔ 企微号是 N-N。本领域两个列表页**不直接展示客服**,只展示企微号(=管家)。                                                               |
| 玩家×企微号关系记录           | 底层数据形态;每条记录承载该玩家在某个企微号上的独立备注、描述、标签、添加时间、删除时间、关系状态(企微原生字段都挂在这一层,跨号不互通)。**在 `/players` 列表中以玩家维度聚合**(同一玩家被 N 个企微号加 → 主表只出 1 行,跨号字段以去重 chip 聚合);具体每条关系的细节去 `/players/:id` 跨号关系 tab 看完整版。 |
...
| 关系状态                 | (玩家,企微号)关系的当前状态,枚举 `正常` / `被管家删除`(客服侧主动删玩家)/ `被玩家删除`(玩家侧拉黑或删客服)。被删除后历史关系数据保留(只是不再能主动发起);玩家管理列表默认只显示 `正常`。                                                                     |
...
| 会话轮次(round)         | 同一 `conversationId` 内由 system「本次会话已结束」边界切出的一段(已结束→重开 = 新一轮)。**`/messages` 以轮次为最小展示单位**,标识 `roundId = conversationId#N`(N 从 1 起,**单轮也带 `#1`**,纯展示派生、非持久化)。工作台 / 玩家详情页仍按整会话维度,轮次不影响其会话模型。 |
...
- Mock 关系记录:每个真人随机被 1-3 个企微号添加,在玩家管理列表里展开为 N 行。
...
| 实体  | 字段                                                 | 类型 / 取值                                             | 说明                                                            |
| --- | -------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------- |
| 玩家  | `playerId`                                         | string                                              | 真人主键(底层 `external_userid`);UI 展示为"playerId",支持复制              |
...
| 关系  | `accountId`                                        | string                                              | 关联的企微号 ID;UI 展示其企微号名,文案为"管家"                                  |
...
| 关系  | `addedAt` / `updatedAt`                            | datetime                                            | 添加 / 更新时间,列表默认按 `updatedAt` 倒序                                |
...
1. `**/players` 玩家管理列表页**(顶栏 tab"玩家管理")
  ...
  - **不再触发会话只读 Drawer**:运营想看会话历史 → 详情页"会话记录" tab → 选会话 → Drawer
2. `**/players/:id` 玩家详情页**(独立路由,V1 引入)
  - **职责**:真人级容器,以 `playerId` 为主键,跨号聚合;为后续大量游戏侧数据预留 tab 容器
  - **页头**:玩家头像 / 微信昵称 / playerId(等宽 + 复制图标)/ 跨号汇总(被 N 个企微号添加 / 总会话数 / 最早添加时间)
  - **tab 切换**(V1 双激活 tab + v1.1+ 占位 tab):
    - **基础**:真人级自定义信息(单一文本字段,可编辑)/ 备注 + 描述聚合视图(每个企微号一段,只读;详细编辑去"跨号关系与会话" tab)
    - **跨号关系与会话**:每个 `(playerId, accountId)` 关系记录列表,每行一个企微号,**关系字段(备注 / 描述 / 标签)内联编辑** + **会话索引列**(消息数 / 最近消息) + 行操作(有会话 → 查看会话;无会话 → 主动发起;被玩家删除 → 主动发起被禁用);**关系级字段的主要编辑入口**(配合工作台 slot 当前管家编辑入口);数据与 `/players` 列表行同源,任一处改动跨页生效
    - **(v1.1+)游戏数据**:V1 占位 tab,显示"待 v1.1+ 接入" + 字段清单预告(游戏 UID / 所属服 / 充值档位 / 最近登录 / VIP 等级 等)
...
4. **会话只读 Drawer**(玩家管理列表 / 玩家详情页"会话记录" tab / 消息管理列表 三处共用)
  ...
  - 底部按钮:"去工作台接待 →" → 跳 `/workbench?conversationId=<id>`(若工作台不存在该会话或会话已结束,工作台进入"主动发起会话"态,语义与 chat-workbench 已落地的"已结束 → 主动发起 → 发送成功才落库"一致)
5. **chat-workbench 右列 slot — 接待专用精简档案**(由本领域提供独立 React 组件,**不复用 `/players/:id`**)
  ...
  - 内容范围**只装与本次接待强相关的字段**:
    ...
    - 关键自定义字段(2-3 个,如游戏 UID / VIP 等级 / 充值档位 — V1 mock,v1.1+ 来自游戏数据)
...
- **本领域所有页面只读、发消息必跳工作台**:玩家管理列表 / 玩家详情页 / 消息管理列表 / Drawer 四处都不渲染输入区;"发消息 / 接待"语义落在工作台 — 已存在的会话直接进入接待,新会话或已结束会话进入"主动发起"态,**首条消息发送成功才落库**。这条与 chat-workbench D1 决策一致,不重复造一套。
...
- **看玩家完整档案**:行操作"查看完整档案" → 跳 `/players/:id` → 三 tab(基础 / 跨号关系 / 会话记录) + v1.1+ "游戏数据" 占位 tab。**关系级细节(备注 / 描述 / 删除时间)与会话浏览均在详情页内完成**。
- **从详情页打开会话历史**:`/players/:id` "会话记录" tab → 点会话 → 同一个会话只读 Drawer(在详情页右侧滑出);Drawer 关闭后留在详情页。**`/players` 列表不再触发 Drawer**(行操作中"查看会话"已移除,2026-05-30)。
- **消息管理筛选 + 浏览**:进入 `/messages` → 顶部筛选(对话管家 / 消息时间范围 / 消息内容 等) → 命中会话表格 → 点行打开 Drawer 浏览消息。
- **跨页跳转**:`/players` 列表 / `/players/:id` "会话记录" tab / `/messages` 三处打开的 Drawer 是**同一个组件实例**;关闭 Drawer 不会丢宿主页面的筛选 / tab 状态。
- **跳工作台接待**:Drawer 底部"去工作台接待 →" → 跳 `/workbench?conversationId=<id>`,工作台拿到 query 后定位会话(已结束 → 进入主动发起态;新会话 → 创建会话占位 + 主动发起态);消息发送成功后会话才在 `/messages` 列表里显示。
- **工作台 slot 流程**:chat-workbench 选中会话 → 右列加载本领域**精简档案组件**(独立组件,非详情页) → 渲染当前管家上的关系字段 + 跨号 chip + 关键自定义字段 → 客服在 slot 内直接改备注 / 描述 / 标签;若需看完整档案点"在玩家管理打开" → **新标签页**打开 `/players/:id`(避免打断接待)。
...
| 事项             | 来源 R* | 说明                                                                 | 阻塞                       |
| -------------- | ----- | ------------------------------------------------------------------ | ------------------------ |
| 会话只读 Drawer    | 追加    | 玩家详情页"会话记录" tab + 消息管理列表共用的右侧抽屉;消息时间 / 内容 / 发送方筛选;"去工作台接待"按钮                    | 阻塞详情页"会话记录" tab 与 `/messages` 行操作"查看对话消息";`/players` 列表本轮已不再触发 Drawer |
...
| 事项                        | 来源  | 说明                                                                                                                                                           | 阻塞                                       |
| ------------------------- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- |
| 玩家管理列表页(`/players`)       | 追加  | 顶栏 tab"玩家管理";筛选条 + 关系状态分类 + **玩家维度表格**(纯只读、不展开,跨号字段 chip 聚合)+ 行操作仅"查看完整档案";关系级编辑入口落在详情页与工作台 slot;"查看会话"动作下放到详情页"会话记录" tab                                                                   | 顶级页面入口,玩家级检索 + 扫读的承载                     |
| **玩家详情页(`/players/:id`)** | 追加  | 真人级容器,以 `playerId` 为主键,跨号聚合;V1 三 tab(基础 / 跨号关系 / 会话记录) + v1.1+ 占位 tab(游戏数据);消息只读 Drawer 在 tab 内右侧滑出;支持深链分享,工作台 slot 跳转走新标签页                                  | 顶级页面入口,**为后续游戏数据预留 tab 容器**,与 slot 不复用组件 |
| 消息管理列表页(`/messages`)      | 追加  | 顶栏 tab"消息管理";筛选条 + 会话表格 + 行操作"查看对话消息";支持 `?conversationId=<id>` 深链                                                                                           | 顶级页面入口,会话级检索的承载                          |
| 工作台 slot 接待专用精简档案         | 追加  | chat-workbench 右列独立组件(**不复用详情页**);只装与本次接待强相关字段:头像 / 昵称 / 当前管家备注 + 描述 + 标签(可编辑)/ 关系状态徽标 / 跨号 chip(只显示不展开) / 关键自定义字段(2-3 个)/ "在玩家管理打开"链接(新标签页跳 `/players/:id`) | 阻塞 chat-workbench 右列升级;边界要清晰             |
...
- **接 chat-workbench(主动发起对接)**:Drawer 底部"去工作台接待"按钮按是否已有会话分两种 query:
  - **已存在会话**(Drawer 是从某个 conversationId 打开的)→ 跳 `/workbench?conversationId=<id>`,工作台定位该会话(进行中 → 直接接待;已结束 → 进入主动发起态)
  - **新会话**(玩家管理行点"查看会话"且该(玩家,企微号)从未产生会话,Drawer 空态下点"去工作台主动发起")→ 跳 `/workbench?playerId=<pid>&accountId=<aid>`,工作台据此创建会话占位 + 主动发起态
  - 两种 query 下"首条消息发送成功才落库"的语义都由 chat-workbench 的 D1 决策承担,本领域不重复实现。
...
> D1(骨架)落定:领域结构 / 页面清单 / 业务流程图 / 共享规则 / 设计例外。
> D2(主页面详设:`/players`、`/players/:id`、`/messages`)与 D3(辅助模块详设:会话只读 Drawer、工作台 slot 精简档案)待补。
...
| 玩家详情页 | 真人级容器,跨号聚合,可扩展 tab | 玩家管理行操作"查看完整档案" / 工作台 slot "在玩家管理打开"(新标签页) | 本领域玩家 + 关系 mock store / chat-workbench 会话反查 | 内嵌"基础 / 跨号关系 / 会话记录" tab + v1.1+ "游戏数据"占位 tab;"会话记录" tab 内打开会话只读 Drawer | 路由 `/players/:id`,支持深链 |
| 消息管理列表页 | 会话级检索 | 顶栏"消息管理" tab | chat-workbench 会话存档 / permission 授权企微号 | 行操作"查看对话消息"打开会话只读 Drawer | 路由 `/messages`,支持 `?conversationId=<id>` 深链 |
| 会话只读 Drawer | 在不打断当前页面的前提下浏览某会话的全部消息 | 玩家管理 / 玩家详情跨号关系与会话 tab / 消息管理 三处行操作 | chat-workbench 会话存档 + 消息气泡组件复用 | 底部"去工作台接待"按宿主上下文跳 chat-workbench(已存在会话 / 主动发起 两条 query) | 三处宿主共用同一组件实例,720~880px |
...
| 玩家与关系 mock store | 本领域唯一权威源,跨页面同步备注 / 描述 / 标签 / 关系状态 | 由列表 / 详情 / slot 三处共同读写 | mock seed(8-12 真人 + 1-3 关系) | 写入后即时广播,所有视图刷新 | V1 不接企微 API,只展示"已同步"标识 |
...
- **内部依赖**:玩家管理列表 / 玩家详情 / 工作台 slot 三处对关系字段(备注 / 描述 / 标签 / 关系状态)同源同步,共用同一份 mock store;任一处编辑,跨页面实时一致。
...
| 玩家详情页 | `/players/:id` | 运营 / 客服 | 真人级完整档案查看与编辑 | 页头(头像 / 昵称 / playerId / 跨号汇总)/ tab 容器 | V1 三 tab:基础 / 跨号关系 / 会话记录;v1.1+ 加"游戏数据"tab |
| 消息管理列表页 | `/messages` | 运营 / 客服 | 会话级检索 + 浏览 | 顶部筛选条 / 会话表格 / 行操作"查看对话消息" | 支持 `?conversationId=<id>` 深链 |
| 会话只读 Drawer | 在 `/players` / `/players/:id` / `/messages` 内触发 | 运营 / 客服 | 浏览某会话的消息历史 | 头部 / 二级筛选条 / 消息列表 / 底部"去工作台接待"按钮 | 三处宿主共用同一组件实例,不占独立 URL |
| 工作台 slot 精简档案 | 在 `/workbench` 内右列挂载 | 客服 | 接待场景下的玩家精简档案 | 头像 + 昵称 / 当前管家关系字段 / 跨号 chip / 关键自定义字段 / "在玩家管理打开"链接 | 独立组件,不复用 `/players/:id` |
...
- 跨页面 tab 切换不卸载对方页面状态(保留筛选条件、滚动位置、详情页 tab 选中);从 Drawer 关闭回到宿主页面时筛选 / tab / 滚动位均保留。
...
```mermaid
graph LR
    A[在 Drawer 内点 去工作台接待] --> B{Drawer 当前是否绑定 conversationId?}
    B -- 是 已存在会话 --> C[跳 workbench<br/>带 conversationId 定位会话]
    C --> C1[chat-workbench 定位会话]
    C1 --> C2{会话状态}
    C2 -- 进行中 --> C3[直接接待]
    C2 -- 已结束 --> C4[进入主动发起态<br/>输入区解锁]
    B -- 否 空态 新会话 --> D[跳 workbench<br/>带 playerId 与 accountId 创建占位]
    D --> D1[chat-workbench 创建会话占位]
    D1 --> D2[进入主动发起态<br/>输入区解锁]
    C4 --> E[首条消息发送成功才落库<br/>由 chat-workbench D1 决策承担]
    D2 --> E
```
...
- `/players` 顶部分类切换条单选三态,**默认 `normal`**;非 `normal` 分类下增加"删除时间"列。
- 被删除关系**保留历史数据**,关系记录、备注 / 描述 / 标签、历史会话与消息均可只读浏览。
- `removed_by_player`(被玩家删除)→ 工作台 slot 顶部醒目警示,直接禁用"主动发起会话"按钮(对齐 chat-workbench D1 决策)。
- `removed_by_agent` 不影响主动发起按钮可用性(管家可重新加回),但工作台 slot 顶部以橙色提示"已被管家删除,继续接待请确认意图"。
...
- 玩家详情页 `/players/:id` "跨号关系" tab 仅展示**可见关系**;真人在不可见企微号上的关系记录被剔除,但页头"跨号汇总(被 N 个企微号添加)"仍按真实总数展示,以避免对运营的扫读误导(具体数字与展开列表可见性的冲突由 D2 决策)。
...
- 任一处编辑(备注 / 描述 / 标签 / 关系状态)→ 写入 mock store → 广播变更 → 所有视图实时刷新,无需手动刷新页面。
...
- **会话反查源 = 工作台同款集合**(2026-06-03):"会话存档"= chat-workbench 静态 seed **+ 已落库的主动发起会话**。V1 实现上两者都在前端:seed 来自 `chatflowMock`,已落库主动发起会话来自共享持久化模块 `src/services/proactiveStore.ts`(localStorage,落库判定 = 非 seed 会话有 ≥1 条 sent 的 outgoing 消息)。player-center 的会话索引 / 轮次 / 消息反查**懒计算合并两者**,确保 `/players/:id` 总会话、跨号关系 tab"查看会话/主动发起"、`/messages` 列表、会话只读 Drawer 与工作台左列**会话集合完全一致**(此前只读 seed 会漏掉落库的主动发起会话,如大海)。真实链路对应会话存档 API 的全量查询。

### 4. 读写分离(本领域所有页面只读 + 发消息必跳工作台)

- `/players` / `/players/:id` / `/messages` / 会话只读 Drawer 四处**均不渲染输入区**,不能发消息、不能结束会话、不能撤回。
- 关系字段(备注 / 描述 / 标签 / 关系状态)的编辑入口仅在两处:`/players/:id` 跨号关系 tab(对应任一管家)+ 工作台 slot 精简档案(对应当前接待管家);`/players` 列表行不暴露编辑入口。
- 自定义字段编辑入口:`/players/:id` 基础信息 tab + 工作台 slot(只暴露关键 2-3 个);`/players` 列表不暴露。
...
- 三处宿主(玩家管理列表 / 玩家详情跨号关系与会话 tab / 消息管理列表)**共用同一个 React 组件实例**,确保交互行为、筛选条、消息渲染完全一致。
...
- 主体消息列表**复用 chat-workbench 消息气泡组件**(继承气泡 8px / 行高 1.55 视觉例外),保证三处视觉一致。
- 底部按钮:"去工作台接待 →",按宿主上下文走两条 query 分支(见图 2)。
- 不渲染输入区、不显示发送按钮、不显示撤回按钮。
- 空态:从未产生会话 → 提示 + "去工作台主动发起"按钮(走新会话 query)。

### 6. 工作台 slot 协议

- 由本领域提供独立 React 组件 `<PlayerSlotPanel playerId accountId />`,挂载在 chat-workbench 右列;**不复用 `/players/:id`**。
- 上下文:chat-workbench 传入 `playerId + accountId`(当前接待企微号),slot 据此渲染"当前管家关系字段 + 跨号 chip + 关键自定义字段"。
- 字段范围(只装与本次接待强相关):头像 / 昵称 / 当前管家备注(可编辑)/ 描述(可编辑)/ 标签(可增删)/ 关系状态徽标 / 跨号关系 chip(只显示其他企微号名,**不展开**)/ 关键自定义字段 2-3 个。
...
- **从未产生会话**:`/players/:id` 跨号关系与会话 tab,无会话的关系行"会话"列显示"暂无会话",行操作变为"主动发起"(被玩家删除则禁用);整列均无会话(罕见)→ 表格行仍渲染,仅"会话"列全部显示空态文案。
...
> 当前阶段:D2(主页面详设)。三个独立路由 `/players`、`/players/:id`、`/messages` 与两个辅助模块(会话只读 Drawer、工作台 slot 精简档案)分别展开。
...
- **页面出口**:
  ...
  - **不再触发会话只读 Drawer**:想浏览会话历史 → 详情页"会话记录" tab(SessionsTab 已承载)。
...
| 查询区 | 顶部筛选条 | 玩家级 + 关系级 OR 命中 | 文本框 / 下拉多选 / 清空按钮 | 继承 ui-brand 紧凑表单密度;6 字段(去掉了"玩家添加时间范围",留 D3 视使用情况补) |
...
- D2 暂去掉"玩家添加时间范围"筛选(玩家维度下"该玩家任一关系的 addedAt 在范围"语义稍弱);D3 视运营反馈考虑补回。
...
- 行操作不弹二次确认(只读跳转,无副作用)。
- **本页不再触发会话只读 Drawer**;运营想看会话历史 → 详情页"会话记录" tab → 选会话 → Drawer。
...
| 极端数据 | 玩家数 10000+ → 仍能正常分页;聚合计算在前端进行(V1 mock 量级可接受);D3 实施时移到后端 |
...
- **页面目标**:以 `playerId` 为主键、跨号聚合的**真人级容器**;在一个独立路由里完整呈现该玩家的真人级字段(基础 / 自定义)+ 全部可见的(玩家×企微号)关系记录 + 全部可见会话索引。**关系级字段编辑主入口**(备注 / 描述 / 标签 / 关系状态);也是 v1.1+ 游戏数据接入的容器。
- **主要角色**:运营、客服(只读 + 编辑关系字段 + 编辑自定义字段;**不发消息**)。
- **页面入口**:
  - 玩家管理列表行操作"查看完整档案" → 同 tab 跳本页(默认 tab `basic`)。
  - 工作台 slot "在玩家管理打开"链接 → **新标签页**打开本页(避免打断接待)。
  - 直接深链 `/players/:id`(可分享、可收藏);URL query `?tab=basic|relations|sessions|game` 支持深链落定 tab。
- **页面出口**:
  - 浏览器返回 → 回到上一个路由(从列表跳来则回列表;新标签页打开则关闭即可)。
  - "会话记录" tab 内点行 → 在该 tab 右侧滑出会话只读 Drawer(三处共用同一组件实例);Drawer 关闭后回到本 tab 同一行。
  - Drawer 底部"去工作台接待 →" → 跳工作台,语义按已存在会话 / 主动发起两条 query 分支(D1 §5)。
...
graph TD
    ...
    F -- 切到游戏数据占位 tab --> L[展示 v1.1+ 占位卡片<br/>列出未来字段清单]
    L --> F
```
...
- **处理**
  ...
  - 跨号汇总计算:
    ...
    - `N_total` = mock store 中该 `playerId` 的真实关系总数(含不可见)。
    - `M_invisible` = `N_total - N_visible`(我不可见的数量)。
...
| Tab 头 | 内容容器切换器 | 切换 4 个 tab | Tab(`type=line`,继承 ui-brand)+ 后置计数(`关系 (N)` / `会话 (N)`)+ v1.1+ 占位 tab 加角标 | Tab 高继承 ui-brand 默认 |
...
| 跨号汇总徽章 - 关系数 | `N_visible` + `M_invisible`(见 2.3) | 主徽章"被 N 个企微号添加",`M_invisible > 0` 时附加灰色"+M 不可见" | 否 | hover 灰色徽章 → Tooltip:"该玩家在 M 个你无权限的企微号上有关系记录,请联系管理员授权";`M_invisible = 0` 时只显示主徽章 |
| 跨号汇总徽章 - 总会话数 | 仅 `visibleRelations` 衍生 | "总会话 N" 徽章 | 否 | 计数与"会话记录" tab 后置计数一致 |
...
- 当 `M_invisible > 0` → 额外加灰色"+M 不可见"次徽章作为透明信号,让运营知道存在更多关系且需授权。
- 跨号关系 tab 内仅展示 `visibleRelations`(与该策略一致);不可见关系不出现在 tab 内,只在徽章中计数。
...
| v1.1+ 占位 tab 表现 | **可点击 + 占位卡片** | 让运营感知容器存在 + 看到未来字段清单;不灰显是因为灰显会被误读为"没有此 tab" |
...
| 操作 | — | 单按钮(三种状态):有会话 → "查看会话"(打开 Drawer);`relationStatus = normal` 且无会话 → "主动发起"(跳工作台 `?playerId&accountId`);`relationStatus = removed_by_player` → "主动发起"灰显禁用 + Tooltip"该玩家已删好友,无法主动发起会话" | 否 | — | 行尾固定列;`fixed: 'right'` |
...
- 编辑冲突(slot 与本 tab 同时改同一字段)→ 后写覆盖,后写完成后双方收到"已被其他端更新"toast,且各自的本地编辑态自动 patch 到最新值。

**行操作语义**:

- **有会话** → 点"查看会话":在该 tab 右侧滑出会话只读 Drawer(同一组件实例,与 `/messages` 复用);Drawer 关闭后回到本 tab 同一行,滚动位与选中态保留。
- **无会话(关系正常)** → 点"主动发起":跳工作台 `/workbench?playerId=<pid>&accountId=<aid>`,工作台据此创建会话占位 + 进入主动发起态;首条消息发送成功才落库的语义由 chat-workbench 承担。
- **无会话(被玩家删除)** → 按钮灰显禁用 + Tooltip 提示无法主动发起。
- **无会话(被管家删除)** → "主动发起"按钮可点(管家可重新加回);跳工作台后语义由 chat-workbench 自身决策。

**可见性与排序**:

- 仅展示 `visibleRelations`;不可见关系不出现在该 tab,但已在页头汇总徽章中计数(2.5.1)。
...
| 行操作 - 主动发起 | 点行尾"主动发起"按钮(行无会话且关系状态非 `removed_by_player`) | 跳工作台 `/workbench?playerId=<pid>&accountId=<aid>` | 工作台创建会话占位 + 进入主动发起态 | 路由失败 → toast 提示,详情页保留 |
...
| 该真人无可见关系(`N_visible = 0`,但 `M_invisible > 0`) | 页头汇总徽章数字显示 0 + 灰色"+M 不可见";跨号关系与会话 tab 空态:"你当前可见的企微号与该玩家无关系,联系管理员授权" |
| 该真人完全无关系(`N_total = 0`)| 等同 playerId 不存在,走错误态(实际不会出现:无关系不会被 mock seed 给 `playerId`)|
| 该真人有可见关系但部分关系无会话 | 跨号关系与会话 tab 正常渲染所有关系行;无会话的行"会话"列显示"暂无会话",行操作变为"主动发起"(被玩家删除则禁用) |
| `customNote` 字段缺失或损坏(mock seed 异常)| 字段降级为空字符串 + 灰色 placeholder 提示"数据异常";不阻塞跨号关系 tab 编辑 |
| 跨号关系 tab 内联编辑冲突 | slot 与本 tab 同时改同一字段 → 后写覆盖;两端各自收到"该字段已被其他端更新,已应用最新值"toast;本地编辑态自动 patch 到最新值,避免覆盖丢失 |
...
| 权限受限 | 仅可见关系出现;不可见关系仅在汇总徽章计数,跨号关系与会话 tab 不渲染对应行 |
...
- **页面目标**:把所有可见(玩家×企微号)关系衍生的会话集中成一张可筛选的扫读表;运营 / 客服在一个屏幕里按"对话管家 / 玩家备注 / 标签 / 消息时间 / 消息内容"等条件检索目标会话,行操作"查看对话消息"在右侧滑出会话只读 Drawer 浏览历史消息。**纯只读检索台**,不渲染输入区。
- **主要角色**:运营、客服(只读 + 跳转,**不发消息**)。
- **页面入口**:顶栏一级 tab "消息管理"(顺序排在"玩家管理"之后);也支持 `?conversationId=<id>` 深链 — 进入页面后**自动打开 Drawer 加载该会话**,常用于跨域 / 通知中心 / 工单系统等外部链接直达消息历史。
- **页面出口**:
  - 行操作"查看对话消息" → 在右侧滑出会话只读 Drawer(三处共用同一组件实例;宿主表格状态保留)。
  - Drawer 底部"去工作台接待 →" → 跳工作台,语义按已存在会话 / 主动发起两条 query(D1 §5)。
- **本页负责**:筛选条件管理、(玩家×企微号)会话索引表渲染、行操作分发(开 Drawer)、`?conversationId=<id>` 深链直达 Drawer、超权限会话前端守卫(假定后端已过滤)。
- **本页不负责**:渲染输入区、暴露字段编辑入口(备注 / 描述 / 标签)、跨真人聚合(同一玩家被多个企微号添加 → 在本页出 N 行,与 `/players` 列表粒度一致)、维护标签库 / 自定义字段定义。
...
| 浮层区 | 会话只读 Drawer | 浏览选中会话的消息历史 | 见 D3 模块章节 | 三处宿主共用同一组件实例;宿主表格状态保留 |

**1440 基线下列宽分配建议**(总宽 ≈ 1392,扣两侧 16px padding):

| 列 | 比例 | 建议宽 (px) | 备注 |
| --- | --- | --- | --- |
| conversationId | 13% | 180 | 等宽字体 + 复制图标 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 会话标识 | `conversationId` | Input(等宽字体) | 精确 | 空 | — | 是(会话轮次列) | 支持粘 `conversationId`(命中其全部轮次行)或 `roundId`(命中单轮行) |
...
| 列字段 | 数据来源 | 截断 / 格式 | 排序 | 列宽建议 | 备注 |
| --- | --- | --- | --- | --- | --- |
| 会话轮次 | `roundId`(= `conversationId#roundIndex`) | 等宽字体字符串(如 `c_006#2`,**单轮也带 `#1`**)+ `roundCount>1` 时附次文本 `/N` + 后置 📋 复制图标 | 否 | 200 | 点复制 → toast"已复制会话标识"(复制 `roundId`) |
...
| 最后消息 | `conversation.lastMessage.{senderType, sentAt, contentPreview}` | 第一行:发送方徽标(玩家 / 客服)+ 时间(`MM-DD HH:mm`);第二行:内容预览(单行省略,~30 字) | 升 / 降(默认 desc) | 420 | 列头默认带降序箭头;两行布局,行高约表格行高的 1.2 倍 |
| 消息总数 | 反查统计 | 数字(对齐右) | 升 / 降 | 85 | — |
...
| 按钮名 | 显示条件 | 触发行为 | 反馈 |
| --- | --- | --- | --- |
| 查看对话消息 | 始终显示 | 在右侧滑出会话只读 Drawer:绑定该轮 `(conversationId, roundIndex)`,**只加载该轮消息**(含该轮自带 system 分隔条);若 Drawer 已开则切换(不重复滑入动画) | 滑入 ~250ms;Drawer 主体 loading 盖层;头部展示玩家昵称 + 对话管家 + `roundId`(如 `c_006#2`)+ 复制图标 |
...
- **三处宿主共用同一组件实例**:
  - `/players` 行操作"查看会话"(入口上下文 = `(playerId, accountId)`,Drawer 自动定位最近一条会话或进入空态)。
  - `/players/:id` "会话记录" tab 行操作"查看消息"(入口上下文 = `conversationId`)。
  - `/messages` 行操作"查看对话消息" + `?conversationId=<id>` 深链(入口上下文 = `conversationId`)。
- **主要角色**:运营、客服(只读浏览,**不发消息、不结束会话、不撤回**)。
- **不渲染输入区、不显示发送按钮、不显示撤回按钮**:这是本领域 D1 §4 读写分离硬性约束,与三个宿主页面完全一致。
- **本模块负责**:头部上下文展示、二级筛选条、消息列表渲染(复用 chat-workbench 气泡组件)、空态 / 错误态、"去工作台接待"按钮按宿主上下文走两条 query 分支。
...
graph TD
    A[宿主页面行操作触发] --> B{入口上下文}
    B -- playerId 加 accountId --> C1[反查最近一条会话<br/>来自 chat-workbench 存档]
    B -- conversationId --> C2[直接定位该会话]
    C1 --> D{是否存在会话}
    D -- 否 --> D1[Drawer 空态<br/>提示 去工作台主动发起]
    D -- 是 --> E[加载该会话消息流]
    C2 --> E
    ...
    G -- 点 去工作台接待 --> K{是否绑定 conversationId}
    K -- 是 --> K1[跳工作台 带 conversationId]
    K -- 否 来自 playerId 加 accountId 空态 --> K2[跳工作台 带 playerId 与 accountId]
    K1 --> L[首条消息发送成功才落库<br/>由 chat-workbench D1 承担]
    K2 --> L
```
...
- **输入**
  - 宿主上下文:由触发方传入,二选一格式:
    - `{ playerId, accountId }`(来自 `/players` 行操作"查看会话")。
    - `{ conversationId }`(来自 `/players/:id` 跨号关系与会话 tab、`/messages` 行操作、`/messages` `?conversationId=<id>` 深链)。
  - 二级筛选:消息时间(范围)/ 消息内容(模糊)/ 发送方(玩家 / 客服 / 全部,默认全部)。
  - 当前客服可见企微号集合:来自 `permission` 接口;Drawer 内部假定后端已过滤,自身不再守卫。
  - 滚动位:首次打开默认滚动到最新消息(底部),用户向上滚动时按需加载历史。

- **处理**
  - 入口上下文解析:
    - `{ playerId, accountId }` → 反查 chat-workbench 会话存档,取该(玩家,企微号)最近一次会话 → 拿到 `conversationId`;若无任何会话 → 进入空态(不再后续加载消息)。
    - `{ conversationId }` → 直接定位该会话;若超权限 / 不存在 → 进入"会话不可见或不存在"错误态。
  - 消息加载:首次加载最近 50 条;向上滚动到顶部时按需加载更多(每次 50 条);**所有消息来自 chat-workbench 会话存档,本领域不重存**。
...
- **输出**
  ...
  - 跳工作台 query 分发(已存在会话 → `?conversationId=<id>`;空态新会话 → `?playerId=<pid>&accountId=<aid>`)。
...
| 主体内容区 | 消息列表 | 浏览消息历史 | 时间分隔条 / 玩家气泡(左)/ 客服气泡(右)/ 轮次分隔条 / 消息状态图标 | **复用 chat-workbench 消息气泡组件**(继承气泡 8px / 行高 1.55 例外);默认底部对齐;向上滚动加载更多 |
| 底部 | 行动按钮区 | 把"想接待"意图下钻到工作台 | "去工作台接待 →" 按钮(主色填充,继承 ui-brand 按钮高度) | 高 48px;按钮按宿主上下文走两条 query(见 4.5.4) |
...
| 跳转语义(已存在会话)| Drawer 当前绑定 `conversationId` → 跳 `/workbench?conversationId=<id>`;chat-workbench 拿到 query 后定位会话(进行中 → 直接接待;已结束 → 进入主动发起态,见 chat-workbench D1) |
| 跳转语义(空态新会话)| Drawer 是从 `/players` 空态打开(入口上下文 = `playerId + accountId` 且无任何会话)→ 跳 `/workbench?playerId=<pid>&accountId=<aid>`;chat-workbench 创建会话占位 + 进入主动发起态 |
| 落库语义 | 两条 query 下"首条消息发送成功才落库"的语义都由 chat-workbench D1 决策承担,本模块不重复实现 |
...
| 场景 | 触发 | 系统处理 | 成功结果 | 失败 / 异常 |
| --- | --- | --- | --- | --- |
| 滑入打开 | 宿主页面行操作触发 | 解析入口上下文 → 反查会话(若 `playerId+accountId`)/ 直接定位(若 `conversationId`)→ 加载最近 50 条消息 | Drawer 滑入 + 头部 + 消息列表 + 底部按钮渲染;默认滚动到底部(最新消息) | 上下文 = `playerId+accountId` 且无会话 → 进入空态;`conversationId` 不存在 / 超权限 → 错误态(4.7) |
...
| 滚动加载更多 | 滚动到顶部 100px 内 | 反查 chat-workbench 取更早 50 条消息 → 拼接到列表前部;保留当前滚动位 | 顶部增量内容显示;无更多消息时显示"已到达最早的消息" | 反查失败 → 顶部错误条 + "重试"按钮 |
...
| 点击底部"去工作台接待" | 点击按钮 | 按宿主上下文判定走两条 query(4.5.4) | 跳工作台 | 路由失败 → toast,Drawer 保留 |
...
| 场景 | 表现 |
| --- | --- |
| 空态 - 从未产生会话 | 入口上下文 = `playerId + accountId` 且该(玩家,企微号)无任何会话 → Drawer 主体显示 `Empty` 插图 + 文案"该玩家与该企微号暂无会话记录" + "去工作台主动发起"按钮(点击带 `?playerId&accountId` 跳工作台);**头部不显示 conversationId**;**底部"去工作台接待"按钮替换为"去工作台主动发起"** |
...
| 加载态 - 滚动加载更多 | 顶部行级 Spin + 文案"加载历史消息中..." |
...
| 极端数据 - 单会话消息 10000+ | 仅首屏加载 50 条,滚动按需加载;客户端筛选用虚拟滚动(D3 实现细节)防止 DOM 爆炸 |
| 全文索引失败 | "消息内容"模糊筛选不可用 → 顶部告警"消息内容索引暂不可用,请使用时间 / 发送方筛选";其他筛选正常 |
| Drawer 打开期间宿主页面变更 | 宿主筛选 / 排序 / 分页变更不主动关闭 Drawer;Drawer 仍绑定原 conversationId(或 playerId+accountId)继续浏览;关闭后回到宿主当前状态 |
...
- **模块目标**:在 chat-workbench 工作台右列提供一份**接待场景专用的玩家精简档案**,只装与本次接待最相关的字段,让客服在不离开工作台的前提下完成"看玩家身份 → 改备注 / 描述 / 标签 → 判断关系状态是否可主动发起"的关键动作。
...
- **本模块负责**:渲染当前管家上的关系字段(可编辑)+ 跨号 chip(只显示不展开)+ 关键自定义字段(可编辑)+ 关系状态徽标 + "在玩家管理打开"链接。
...
graph TD
    ...
    C --> D[渲染头像 昵称 当前管家关系字段<br/>跨号 chip 关键自定义字段]
    D --> E{用户动作}
    E -- 编辑当前管家备注 描述 标签 --> F[校验 写回 mock store<br/>广播刷新列表 与 详情页]
    F --> D
    E -- 编辑关键自定义字段 --> G[校验 写回 mock store<br/>广播刷新详情页基础 tab]
    G --> D
    E -- 点 在玩家管理打开 --> H[新标签页打开 详情页深链]
    E -- 关系状态被玩家删除 --> I[顶部醒目警示<br/>chat-workbench 据此禁用主动发起]
    I --> D
    E -- 切换接待会话 --> A
```
...
- **处理**
  ...
  - 关系状态信号:将 `relation.relationStatus` 透出给 chat-workbench(通过组件内部回调或全局 context),chat-workbench 据此决定"主动发起会话"按钮开关。
...
├──────────────────────────────────────────┤
│ 玩家身份卡 (含警示横幅)                  │
│ ⚠ 该玩家已删除好友,无法主动发起接待     │  (仅 removed_by_player 显示)
│ ┌─┐ {昵称}  / {备注}                     │
│ │📷│ playerId: ext_xx [📋]               │
│ └─┘                                      │
├──────────────────────────────────────────┤
...
| 当前管家关系字段 | 备注 / 描述 / 标签 / 关系状态 | 接待中的高频编辑字段 | Input / Textarea / Select 多选 / 徽标 | 与 `/players/:id` 跨号关系 tab 同款编辑控件 |
...
| 警示横幅(被玩家删除) | `relation.relationStatus == removed_by_player` | 顶部条:"⚠ 该玩家已删除好友,无法主动发起接待" + 错误色背景(继承 chat-workbench 风控告警大色块例外) | 否 | 仅 `removed_by_player` 显示;`removed_by_agent` 用橙色提示"已被管家删除,继续接待请确认意图";`normal` 不显示 |
...
**编辑冲突策略**:与 `/players/:id` 跨号关系 tab 编辑同一字段时 → 后写覆盖;两端各自收到"该字段已被其他端更新,已应用最新值" toast,本地编辑态自动 patch。
...
| 关系状态 = `removed_by_player` | mock store 推送状态 → slot 监听 | slot 顶部插入红色警示横幅;通过 context 通知 chat-workbench 禁用"主动发起会话"按钮 | 警示显示 + 主动发起按钮禁用 | — |
| 关系状态 = `removed_by_agent` | 同上 | slot 顶部插入橙色提示横幅("已被管家删除,继续接待请确认意图");**不**禁用主动发起 | 警示显示 | — |
| 编辑冲突(slot 与详情页同时改) | 详情页跨号关系 tab 同步推 | 后写覆盖;slot 收到 toast"该字段已被其他端更新,已应用最新值";本地编辑态 patch | 字段值刷新 | — |
...
| 关系状态 = `removed_by_player` | 顶部红色警示横幅;关系字段(备注 / 描述 / 标签)**仍可编辑**(允许补充关系标记便于复盘);自定义字段仍可编辑;主动发起按钮由 chat-workbench 禁用 |
| 关系状态 = `removed_by_agent` | 顶部橙色提示横幅;所有编辑入口仍可用 |
...
| 跨企微号编辑同步延迟 | 在 mock store 广播未到达前,slot 显示自己写入的最新值;广播到达后若值不一致(冲突)→ patch + toast |
