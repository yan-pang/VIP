...
- 顶栏一级 tab 在 player-center 启用时追加两项:**玩家管理 → `/players`** 与 **消息管理 → `/messages`**(顺序排在“客服工作台 / 运营管理控制台”之后；控制台仍保留一级快捷入口)。
...
- **页面入口**:顶栏一级 tab "玩家管理"(顺序排在“客服工作台 / 运营管理控制台”之后);也支持深链参数(关系状态 / 管家 / playerId 等)直接落定筛选状态。
- **页面出口**:
