...
> - 功能详细描述：模块关系、权限、共享状态、企微号配置、资源管理、违禁词、风控、审计与应急
> - 设计例外说明：项目级视觉规范的例外
...
**一句话目标**：为 ChatFlow 提供企微号、云桌面绑定与违禁词的受控初始化配置，让工作台和 permission 可以读取同一套运营基础数据，而不在业务页面分散维护。

1. 系统管理员维护企微号基础信息、所属游戏和云桌面绑定；游戏目录本身仍为只读主数据，不在本领域建设 CRUD。
2. 系统管理员维护客服向玩家发送消息时使用的违禁词规则；玩家来消息不做拦截。
3. 将 RPA 风控红线固化为配置校验与运维状态：固定 IP、单号单桌面、发送频率、脱敏边界和应急回退。
...
| --- | --- | --- | --- | --- |
| 企微号配置页 | 维护企微号、所属游戏、资源绑定，展示企微账号状态和风控摘要 | 顶栏“运营管理” | 只读游戏目录、共享号 Store | 供 permission / 控制台 / 工作台读取；不负责登录或投屏 |
...
- **入口**：顶栏“运营管理” → “企微号配置”。
- **出口**：进入配置抽屉或云电脑 & RPA 机器人管理页；控制台只承担已授权号的投屏与运维。
- **负责**：企微号基础资料、企业主体、所属游戏、资源绑定、生命周期与风控状态展示。
- **不负责**：游戏目录 CRUD、客服账号与号授权、企微扫码登录、投屏会话和真实无影云调用。
