...
3. 如果给了具体领域，再按需读 `project/domains/<domain>/design.md`（先章节索引再跳读）、`decisions.md` 和自动化契约测试；已有 `delivery/` 只作为历史快照按需读取。
