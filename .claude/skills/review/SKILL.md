...
3. 项目级评审时,优先检查 `project/TASKS.md`(运行时状态单一事实源)、`project/overview.md`(项目地图)、`project/ui-brand.md` 是否与领域文档、代码入口、共享能力、项目级调研 / 研发设计 / 版本交付目录一致。
4. 文档评审时,优先检查:
   - `design.md` 是否仍保留"先设计初始化、再补功能详细描述"的主线
   - `design.md` 是否错误地重复维护了 `project/ui-brand.md` 中已经稳定的项目级设计规范
   - `design.md` 是否把运行时状态(带日期决策、迭代历史、待确认项)堆在正文,而未沉淀到该领域 `decisions.md` / `project/TASKS.md`(应分离,保持设计正文纯净)
